...
import com.devices.Lamp;
import com.resource.Definitions;

@Path("devices/actuator/lamp")
public class ArduinoLamp extends Lamp{
	...
	public String getMethod(@PathParam("id") String arg0) {
		...
		try{
	        ...
	        this.setState(value.equals("1") ? "ON" : "OFF");
		}catch(Exception e){
			e.getMessage();
		}
		this.setId(arg0);
	...
	public String postMethod(@PathParam("id") String arg0,
			...
			@DefaultValue("ON") @FormParam("state") String arg2) {
		...
		try{
	        ...
	        arduino.sendComand(DefinitionsArduino.WHITE_INDEX + port + state);
	        
	      	this.setState(arg2);
	      	this.setStatus(arg1);
	      	
	      	System.out.println(DefinitionsArduino.WHITE_INDEX + port + state);
